...
function CostEstimate({ recipe, servings }: { recipe: Recipe; servings: number }) {
  ...
  useEffect(() => {
    (async () => {
      const stores = profile?.preferredStores || [];
      if (!stores.length) { setText('Select stores to see savings'); return; }
      const ap = await getActivePlan();
      const est = await estimateRecipeCost(recipe, servings, stores, 'one');
      if (!est.oneStoreBest) { setText('No price data yet'); return; }
      ...
      if (est.perStore && est.oneStoreBest) {
        const ps = est.perStore.find((s) => s.storeId === est.oneStoreBest?.storeId);
        if (ps && ps.stale) line += ` • ${ps.stale} stale`;
      }
      setText(line);
    })();
